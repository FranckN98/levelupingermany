...
export default function EddyExperience({ initialLanguage = 'fr' }: { initialLanguage?: Language }) {
  ...
  const [selectedEvent, setSelectedEvent] = useState<EventAppearance | null>(null);
  const [submitted, setSubmitted] = useState(false);
  const text = siteData.content[language];
  const settings = siteData.siteSettings;
  const events = [...siteData.events].filter((event) => event.published !== false).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const submitForm = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = [`Hello Eddy, I would like to book you for an event.`, '', ...details.map(([label, value]) => `*${label}:* ${value}`)].join('\n');
    const whatsappHref = settings.whatsappHref || `https://wa.me/${settings.phoneHref.replace(/\D/g, '')}`;
  ...
  return (
    <main id="main" className="overflow-hidden">
      ...
      <section id="home" className="relative isolate flex min-h-[900px] items-end overflow-hidden pb-16 pt-32 sm:min-h-screen sm:pb-20">
        <Image src="/appearances/eddy-hero.jpg" alt="Eddy Gaetan hosting an event with a microphone" fill priority sizes="100vw" className="-z-30 object-cover object-[62%_20%]" />
        ...
        <div className="mx-auto w-full max-w-7xl px-6">
          <div className="max-w-3xl">
            ...
            <div className="mt-7 flex animate-fade-up items-center gap-3" style={{ animationDelay: '360ms' }}>
              <span className="text-[10px] font-bold uppercase tracking-[.16em] text-silver/55">Follow</span>
              {settings.social.map((social) => <a key={social.label} href={social.href} target="_blank" rel="noreferrer" aria-label={social.label} className="border-b border-white/25 pb-1 text-silver-light transition hover:border-electric hover:text-electric"><SocialIcon label={social.label} /></a>)}
            </div>
          </div>
      ...
      {selectedEvent && <div className="fixed inset-0 z-50 overflow-y-auto bg-midnight/90 px-4 py-8 backdrop-blur" role="dialog" aria-modal="true" aria-label={selectedEvent.title}><div className="relative mx-auto max-w-4xl border border-white/15 bg-midnight shadow-2xl"><button type="button" onClick={() => setSelectedEvent(null)} className="absolute right-4 top-4 z-10 bg-midnight/80 p-3 text-white" aria-label={text.appearances.close}><X /></button><div className="relative aspect-[16/7]"><Image src={selectedEvent.cover.src} alt={selectedEvent.cover.alt} fill sizes="(max-width: 900px) 100vw, 900px" className="object-cover object-[50%_20%]" /></div><div className="p-7 sm:p-10"><p className="text-xs font-bold uppercase tracking-[.16em] text-electric">{selectedEvent.role} · {selectedEvent.type}</p><h3 className="mt-4 font-display text-4xl font-semibold text-white">{selectedEvent.title}</h3><p className="mt-6 max-w-2xl text-lg leading-relaxed text-silver/80">{selectedEvent.impact}</p><dl className="mt-8 grid gap-5 border-t border-white/10 pt-6 sm:grid-cols-3"><div><dt className="text-[10px] font-bold uppercase tracking-[.14em] text-silver/50">{text.appearances.date}</dt><dd className="mt-2 text-sm text-white">{new Intl.DateTimeFormat(locale, { dateStyle: 'long' }).format(new Date(selectedEvent.date))}</dd></div><div><dt className="text-[10px] font-bold uppercase tracking-[.14em] text-silver/50">{text.appearances.location}</dt><dd className="mt-2 text-sm text-white">{[selectedEvent.venue, selectedEvent.city, selectedEvent.country].filter(Boolean).join(', ')}</dd></div><div><dt className="text-[10px] font-bold uppercase tracking-[.14em] text-silver/50">{text.appearances.role}</dt><dd className="mt-2 text-sm text-white">{selectedEvent.role}</dd></div></dl></div></div></div>}
    </main>
  );
}
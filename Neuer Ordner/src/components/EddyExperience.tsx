...
import { content, siteSettings, type Language } from '@/data/site-content';
...
export default function EddyExperience() {
  ...
  const navigateGallery = (direction: 1 | -1) => {
    setGalleryIndex((current) => (current + direction + siteSettings.gallery.length) % siteSettings.gallery.length);
  };
      ...
      <section id="home" className="relative isolate flex min-h-[900px] items-end overflow-hidden pb-16 pt-32 sm:min-h-screen sm:pb-20">
        ...
        <div className="mx-auto w-full max-w-7xl px-6">
          ...
          <dl className="mt-16 grid max-w-2xl grid-cols-2 border-t border-white/20 pt-6 sm:mt-24">
            <div className="border-r border-white/20 pr-6"><dt className="font-display text-4xl font-semibold text-white sm:text-5xl"><Counter target={siteSettings.stats.events} /></dt><dd className="mt-2 text-xs font-bold uppercase tracking-[0.16em] text-silver/70">{text.stats.events}</dd></div>
            <div className="pl-6"><dt className="font-display text-4xl font-semibold text-white sm:text-5xl"><Counter target={siteSettings.stats.people} /></dt><dd className="mt-2 text-xs font-bold uppercase tracking-[0.16em] text-silver/70">{text.stats.people}</dd></div>
          </dl>
        </div>
      </section>
